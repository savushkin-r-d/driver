...
class CListImpl : public CWindowImpl< CListImpl< T > >,
    ...
    public:
        ...
        void RegisterClass()
            {   		
            static_cast<T*>(this)->GetWndClassInfo().m_wc.lpfnWndProc = m_pfnSuperWindowProc;
            static_cast<T*>(this)->GetWndClassInfo().Register( &m_pfnSuperWindowProc );
            }

        BOOL Initialise()
            {
            // load list images
            if ( !m_ilListItems.CreateFromImage( IDB_LISTITEMS, 16, 0, 
        ...
        BOOL GetItemDate( int nItem, int nSubItem, SYSTEMTIME& stItemDate )
            ...
            stItemDate.wYear = ( WORD ) _ttoi( strItemText.Left( 4 ) );
            stItemDate.wMonth = ( WORD ) _ttoi( strItemText.Mid( 4, 2 ) );
            stItemDate.wDay = ( WORD ) _ttoi( strItemText.Mid( 6, 2 ) );
            stItemDate.wHour = ( WORD ) _ttoi( strItemText.Mid( 8, 2 ) );
            stItemDate.wMinute = ( WORD ) _ttoi( strItemText.Mid( 10, 2 ) );
            stItemDate.wSecond = ( WORD ) _ttoi( strItemText.Mid( 12, 2 ) );
        ...
        BOOL EnsureItemVisible( int nItem, int nSubItem = NULL_SUBITEM )
            ...
            if ( m_bFocusSubItem && nSubItem != NULL_SUBITEM )
                ...
                if ( rcColumn.Width() > _rcClient.Width() || rcColumn.left < 0 )
                    nScrollPos = rcColumn.left;
                else if ( rcColumn.right > _rcClient.right )
                    nScrollPos = rcColumn.right - _rcClient.right;
        ...
        void DrawGradient( CDCHandle dcPaint, CRect& rcRect, COLORREF rgbTop, COLORREF rgbBottom )
            ...
            TRIVERTEX triVertext[ 2 ] = {
                rcRect.left,
                rcRect.top,
				LOWORD( GetRValue( rgbTop ) << 8 ),
				LOWORD( GetGValue( rgbTop ) << 8 ),
				LOWORD( GetBValue( rgbTop ) << 8 ),
                0x0000,			
                rcRect.right,
                rcRect.bottom,
				LOWORD( GetRValue( rgbBottom ) << 8 ),
				LOWORD(GetGValue( rgbBottom ) << 8 ),
				LOWORD(GetBValue( rgbBottom ) << 8 ),
                0x0000
                };
