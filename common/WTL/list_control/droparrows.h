...
public:
	BOOL Create( HWND hWndParent, int nSpanLength, BOOL bVertical )
	{
        ...
        RECT rect;
        rect = CRect( 0, 0, m_bVertical ? 12 : m_nSpanLength, m_bVertical ? m_nSpanLength : 12 );

		if ( CWindowImpl< CDropArrows >::Create( hWndParent, 
            rect, NULL, WS_POPUP | WS_DISABLED, WS_EX_TOOLWINDOW ) == NULL )
			return FALSE;
