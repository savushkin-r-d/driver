...
public:
	BOOL Create( HWND hWndParent, int nItem, int nSubItem, CRect& rcRect, UINT nFlags, SYSTEMTIME& stItemDate )
	{
        ...
        RECT rect = CRect( rcRect.left + 3, rcRect.top + 2, rcRect.right - 3, rcRect.bottom - 2 );
		if ( CWindowImpl< CListDate, CDateTimePickerCtrl >::Create( 
            hWndParent, rect, NULL, dwStyle ) == NULL )
			return FALSE;
