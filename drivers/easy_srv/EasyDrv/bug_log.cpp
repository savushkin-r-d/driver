...
uintptr_t WINAPI bug_log::bug_log_thread( LPVOID lpParameter )
    ...
    bug_log_window->create();           // Создаем окно приложения.
    is_init_window = 1;
...
int bug_log::add_msg( CString object_name, CString IP4_address, 
    ...
    bug_log_window->add_msg_to_grid( object_name, IP4_address, msg_ );
    return 0;
    }
//-----------------------------------------------------------------------------
int bug_log::add_error_msg( CString object_name, CString IP4_address, 
    ...
    bug_log_window->add_error_to_grid( object_name, IP4_address, msg_ );
    return 0;
    }
//-----------------------------------------------------------------------------
int bug_log::commit_error_msg( CString object_name, CString IP4_address, 
    CString msg_ /*= msg */ )
    {
    bug_log_window->commit_error_msg_to_grid( object_name, IP4_address, msg_ );
    return 0;
    }
//-----------------------------------------------------------------------------
int bug_log::add_warning_msg( CString object_name, CString IP4_address,
    ...
    bug_log_window->add_warning_msg_to_grid( object_name, IP4_address, msg_ );
    return 0;
    }
//-----------------------------------------------------------------------------
...
void bug_log::set_error( char &is_set_error, const char* PAC_name, 
    const char* ip_address, CString msg_ )
    {
    if ( 0 == is_set_error )
        {
...
bug_log::bug_log()
    ...
    StrCatW( path, _T( "\\drv_buglog2.log" ) );

    bug_log_window_thread_handle = chBEGINTHREADEX( 0, 0, bug_log_thread, 0, 0, 0 );	

    if ( bug_log_file.open( path ) )
		{
		this->msg.Format(_T("Не удалось открыть файл логов %s!"),
			path);
		add_warning_msg( "Driver", "" );
		}
...
BOOL list_message_data::Initialise()
    ...
    AddColumn( _T( "Время" ), 60 );
    AddColumn( _T( "Объект" ), 145 );
    AddColumn( _T( "IP адрес" ), 85 );
#ifdef DEBUG
